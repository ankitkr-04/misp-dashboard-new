  ...
  return (
    <main className="h-screen overflow-hidden px-5 pb-5 pt-[138px] lg:pt-[88px]">
      <div className="mx-auto flex h-full max-w-[1800px] flex-col gap-3">
        ...
        <section className="flex shrink-0 items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              {view === "overview"
                ? "Real-time MISP indicator stream and geographic routing of active threats."
                : view === "feed"
                  ? "Incoming threat intelligence records from the active data source."
                  : view === "geography"
                    ? "Source IP routes plotted against active command-centre targets."
        ...
        {/*
         * Overview: Summary bar at top, then 2-col [globe (dominant left) | feed (right)].
         * Analytics panel is NOT shown on overview — it has its own dedicated tab.
         */}
        {view === "overview" ? (
          <>
            <SummaryBar />
            <div
              className="grid min-h-0 flex-1 gap-3
                grid-cols-1
                xl:grid-cols-[minmax(540px,1.65fr)_minmax(300px,0.9fr)]"
            >
              {/* Globe — dominant, left column on xl */}
              <div className="order-1 min-h-0">
                {geographyPanel}
              </div>
              {/* Feed — right column on xl, below globe on mobile */}
              ...
              <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-200 px-5 py-4">
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">
                    Click any record to triage, then open a full investigation.
                  </p>
                </div>
                  ...
                  {[
                    {
                      step: "1",
                      title: "Triage",
                      body: "Review severity, source IP, threat type, and the targeted HQ node.",
                    },
                    {
                      step: "2",
                      title: "Investigate",
                      body: "Open the full investigation page to see raw JSON, tags, route context, and AI analysis.",
                    },
                    {
                      step: "3",
                      title: "Document",
                      body: "Use the investigation workspace to record findings for reporting and evaluation.",
                    },
                  ].map((s) => (
                    <div
                      key={s.step}
